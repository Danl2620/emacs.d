  ...
  (defcustom p4-executable
	...
	(let ((tmp (concat (file-name-as-directory (car lst))
	  ...
	  (if (and (file-executable-p tmp)
		   ...
		   (not (string-match (regexp-quote "(") tmp))
		   (not (string-match " " tmp)))
	      (setq p4ex tmp))
	  (setq lst (cdr lst))))
      p4ex)
...
Does auto re-highlight management (whatever that is)."
    (save-excursion
      ...
      (let ((result
	     ...
	     (apply 'call-process-shell-command (p4-check-p4-executable) nil
		    output-buffer
		    nil			; update display?
  ...
  (defvar p4-menu-def
    '(["Specify Arguments..." universal-argument t]
      ...
      ["Check out/Edit"    p4-edit
       nil] ;; (and (p4-buffer-file-name-2) (or (not p4-mode) buffer-read-only))]
      ["Re-open"	       p4-reopen
...
the VC check on/off when opening files."
  (interactive)
  (p4-set-vc-mode (not p4-do-find-file)))
